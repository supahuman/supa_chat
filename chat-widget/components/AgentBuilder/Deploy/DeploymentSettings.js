...
const DeploymentSettings = ({ selectedAgent }) => {
  ...
  // API hooks
  const { data: deploymentConfig, isLoading: configLoading } = useGetDeploymentConfigQuery(
    selectedAgent?.agentId, 
    { skip: !selectedAgent?.agentId }
  );

  const { data: analytics, isLoading: analyticsLoading } = useGetDeploymentAnalyticsQuery(
  ...
  const handleGenerateCode = useCallback(async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        console.log('📝 Embed code set:', result.data.embedCode.substring(0, 100) + '...');
      } else {
  ...
  const handleTestDeployment = async () => {
    ...
    try {
      const result = await testDeployment({
        agentId: selectedAgent.agentId,
        testMessage: 'Hello, this is a test message from the deployment system!'
      }).unwrap();

      if (result.success) {
        alert('✅ Deployment test successful! Your agent is ready to be embedded.');
      }
    } catch (error) {
      console.error('Deployment test failed:', error);
      alert('❌ Deployment test failed. Please check your agent configuration.');
    }
  };
            ...
            <div className="p-6">
              ...
              <div className="mt-4 p-4 bg-blue-50 dark:bg-blue-900/20 rounded-lg">
                ...
                <ol className="text-sm text-blue-800 dark:text-blue-200 space-y-1 list-decimal list-inside">
                  ...
                  <li>Your AI agent will appear as a chat widget on your website</li>
                  <li>Test the deployment to ensure everything works correctly</li>
                </ol>
              </div>
            </div>
          </Card>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="md:col-span-2">
                  ...
                  <input
                    ...
                    onChange={(e) => setEmbedConfig({ ...embedConfig, customDomain: e.target.value })}
              ...
              <div className="mt-6 space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <input
                    ...
                    onChange={(e) => setEmbedConfig({ ...embedConfig, showWelcomeMessage: e.target.checked })}
            ...
            <Card>
              <div className="p-6">
                <div className="flex items-center">
                  ...
                  <div className="ml-4">
                    <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Satisfaction Rating</p>
                    <p className="text-2xl font-semibold text-gray-900 dark:text-white">
                ...
                <div className="flex items-center justify-between py-2 border-b border-gray-200 dark:border-gray-700">
                  ...
                  <span className="text-sm text-gray-900 dark:text-white">
                    {analytics?.data?.stats?.lastUsed ? 
                      new Date(analytics.data.stats.lastUsed).toLocaleDateString() : 
                      'Never'
                    }
                  </span>
                </div>
