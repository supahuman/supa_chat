    ...
    <div className="space-y-4">
      ...
      {agents.length === 0 ? (
        <div className="text-center py-12">
          <p className="text-gray-500">No agents found. Create an agent to get an API key.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {agents.map((agent) => (
            <div key={agent.agentId} className="border border-gray-200 dark:border-gray-700 rounded-lg p-4">
              <div className="flex items-center justify-between">
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-gray-500 dark:text-gray-400 font-mono">
                    {visibleKeys[agent.agentId] ? agent.apiKey : maskApiKey(agent.apiKey)}
                  </p>
                </div>
                <div className="flex items-center space-x-2 ml-4">
                  <button
                    ...
                    title={visibleKeys[agent.agentId] ? 'Hide API key' : 'Show API key'}
                  >
