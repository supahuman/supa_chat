...
router.post('/webhook', async (req, res) => {
  ...
  try {
    event = PaymentService.getStripe().webhooks.constructEvent(req.body, sig, endpointSecret);
  } catch (err) {
    ...
    case 'checkout.session.completed':
      ...
      if (session.mode === 'subscription') {
        try {
          ...
          console.error(`❌ Error processing checkout.session.completed for session: ${session.id}`, error.message);
...
router.post('/create-checkout-session', async (req, res) => {
  try {
    ...
    const session = await PaymentService.createCheckoutSession(
      ...
      cancelUrl || `${process.env.FRONTEND_URL}/dashboard?upgrade=canceled`
    );
